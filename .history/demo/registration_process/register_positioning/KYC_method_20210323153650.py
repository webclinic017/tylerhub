...
class kyc_approve(Commonweb):
    ...
    def bos_lang(self,lang):
        try:
            if lang=='CN' or lang=='简中':
                ...
                time.sleep(1)
                self.display_click('css,.ivu-select-item',1) #选择页面语言为中文
            else:
                pass
        except Exception as msg:
    ...
    def is_rebate_type(self):
        if self.account[0:2]=='10':
            #点击代理申请
            time.sleep(2)
            self.display_click('css,.el-button--primary')
            time.sleep(2)
            #同意IB协议
    ...
    def get_emailcode_(self):
        try:
            ...
            time.sleep(2)
            #判断验证码邮件是否发送
            if self.ele_is_displayed('xpath,//span[.="emailCode"]',2):
                print(self.ele_is_displayed('xpath,//span[.="emailCode"]',2))
                #打开验证码邮件
    ...
    def china_kyc(self):
        ...
        time.sleep(1)
        self.upload_img(path_process+r'\test_excel_data\card_behind.exe')
        time.sleep(3)
        self.js_scroll('down')
        time.sleep(1)
        #一键填充
