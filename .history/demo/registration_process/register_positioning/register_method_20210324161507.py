...
class form_operations(Commonweb):
    ...
    def browsertype(self,browsername='Chrome'):
        ...
        self.commeth=commonmethod(self.driver) #赋值对象driver
    ...
    def register_topup(self):
        self.commeth.remove_register_topup()

    #选择注册页弹窗语言
    def choose_lg(self,lang):
        """注册页弹窗语言目前只能选择简中和英语"""
        self.commeth.choose_register_lang(lang)
    ...
    def fill_inform(self,lang,fristname,lastname,emali,password,cn_country,en_country):
        ...
        try:
            ...
            #输入名字
            self.display_input('css,.el-input__inner',fristname,1)
            #输入姓氏
            self.display_input('css,.el-input__inner',lastname,2)
            #输入随机手机号码
            self.display_input('css,.el-input__inner',pub_method.get_rangephone(),4)
            #输入电子邮箱
            self.display_input('css,.el-input__inner',emali,5)
            #输入登录密码
            self.display_input('css,.el-input__inner',password,6)
            #输入验证码,并判断验证码是否正确
            self._code_()
            time.sleep(1)
            #点击条款
    ...
    def _code_(self):
        try:
            while True:
                ...
                time.sleep(1)
                #判断验证码是否正确
                if common.ele_is_displayed('css,.el-form-item__error',2):
                    #刷新验证码
    ...
    def register_success(self):
        time.sleep(2)
        try:
            return self.display_get_text('css,div.declar-dialog .el-dialog__title')
        except Exception as msg:
