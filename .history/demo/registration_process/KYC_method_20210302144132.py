...
class kyc_approve(Commonweb):
    ...
    def get_emailcode_(self,account):
        try:
            ...
            time.sleep(1)
            #点击客户名单
            self.web_click('css,.ivu-menu-item-active',1)
            time.sleep(1)
            #根据主账号搜索
            self.web_input('css,.ivu-input-default',account) #输入主账户
            time.sleep(1)
            self.web_click('css,.ivu-icon-ios-search') #点击搜索按钮
            time.sleep(1)
            #点击主账号进入账号详情页
            ...
            if self.is_element_isdisplayed('xpath,//span[.="emailCode"]'):
                ...
                #提取数字
                return pub_method.extract_numbers(acc_text)
            else:
 ...
 tbody.ivu-table-tbody>tr>td>div>div>div
    ...
    def quitdriver(self):
        self.quit_browser()
div.ivu-table-overflowX>table>tbody.ivu-table-tbody>tr>td