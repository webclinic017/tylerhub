...
class review_actions(Commonweb):
    ...
    def ibtype(self):
        try:
            ...
            time.sleep(5)
            print(self.is_element_isdisplayed('css,div.input-item-page-enabled>div>div'))
            while True:
                self.ibtext=self.get_text('css,div.input-item-page-enabled>div>div')
                if 'default' in self.ibtext:
            ...
            time.sleep(3)
            #点击返佣申请表格
            self.display_click('css,.ivu-collapse-header',4)
            time.sleep(1)
            #打开审核开关
