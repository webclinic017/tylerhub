...
class location_withdrawal_incp():
            ...
            if common.ele_is_displayed('css,div.bankItem>div.info',2):#存在支付渠道
                if common.get_lenofelement('css,div.bankItem>div.info')>1:#存在多个取款方式
                    ...
                    if common.display_get_text('css,span.status') in ('审核通过','Approved'):
                        if common.get_attributes('css,.bankInfo > div .ivu-switch input','value',i)=='true':
                            print('出金渠道审核通过')
                        else:
    ...
    def review_withdrawal(self,traccount):
        try:
            ...
            time.sleep(3)
            #勾选该条出金记录
            common.display_click('css,td.ivu-table-column-H8SB8E .ivu-checkbox-input')
            time.sleep(1)
            #转完成
    ...
    #登录会员中心出金，bos审核
    def withdrawal_cp(self,username,psword,traccount,amount):
        try:
            #登录会员中心
            self.logincp(username,psword)
            time.sleep(8)
            #出金
            self.withdrawal_action(traccount,amount)
            #bos审核出金
