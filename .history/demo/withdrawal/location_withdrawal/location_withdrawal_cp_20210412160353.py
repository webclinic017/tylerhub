...
class location_withdrawal_incp():
            ...
            for i in ['localCurrency-card','bank-card','e-wallet-card']:
                if common.ele_is_displayed('css,.{} .isApprove'.format(i),2):
                    if i=='localCurrency-card':
                        withdrawal_type=='当地货币银行转账'
                        print('当前账号可用出金方式为：{}'.format(withdrawal_type))
                        return withdrawal_type
                        break
                    elif i=='bank-card':
                        withdrawal_type=='国际银行电汇'
                        print('当前账号可用出金方式为：{}'.format(withdrawal_type))
                        return withdrawal_type
                        break
                    else:
                        withdrawal_type=='电子钱包'
                        print('当前账号可用出金方式为：{}'.format(withdrawal_type))
                        return withdrawal_type
                        break
                else:
    ...
    def withdrawal_action(self,traccount,amount):
        ...
        try:
            ...
            time.sleep(1)
            common.display_click('xpath,//span[.="{}"]'.format(withdrawal_type))
            time.sleep(1)
            #选择渠道
            common.display_click('css,.form > .left-row > div .el-select__caret',-1)
            time.sleep(1)
            if withdrawal_type=='电子钱包':
                #渠道选择skill时
            ...
            print('当前交易账号可取款金额为：{}'.format_map(self.withdrawal_amount))
            if amount>=self.withdrawal_amount:
                print('取款金额大于可取款金额')
            else:
                pass
