...
class location_withdrawal_incp():
    ...
    def is_wayto_withdrawal(self):
        try:
            ...
            time.sleep(1)
            if common.ele_is_displayed('css,div.bankItem>div.info',2):
                if common.display_get_text('css,span.status') in ('审核通过','Approved'):
                    print('该交易账号存在本地银行出金方式且审核通过')
                else:
                    print('该账号存在本地银行出金方式但未审核')
                    self.creat_local_currency()
            else:
                print('该账号不存在本地银行支付方式')
                self.creat_local_currency()
        except Exception as msg:
    ...
    def creat_local_currency(self):
        """
        居住国为：'中国','中国台湾','中国香港','南韩','越南','日本','中国澳门','菲律宾','马来西亚','印尼','泰国','缅甸'，新增本地货币出金方式
        其他国家新增电子钱包出金方式
        """
        try:
            #基本资料
            ...
            if self.live_country in ('中国','中国台湾','中国香港','南韩','越南','日本','中国澳门','菲律宾','马来西亚','印尼','泰国','缅甸'):
                ...
                time.sleep(1)
                #收款人姓名
                common.display_input('xpath,//div[@class="ivu-drawer-wrap"]//div[@class="ivu-row"]'
                '/div[1]//input[@class="ivu-input ivu-input-default"]','uitest')
                time.sleep(1)
                #银行名称
                ...
                #分行名称
                common.display_input('xpath,//div[@class="ivu-drawer-wrap"]//div[@class="ivu-row"]'
                '/div[1]//input[@class="ivu-input ivu-input-default"]','testbank',2)
                time.sleep(1)
                #分行省份
                common.display_input('xpath,//div[@class="ivu-drawer-wrap"]//div[@class="ivu-row"]'
                '/div[1]//input[@class="ivu-input ivu-input-default"]','testprovince',3)
                time.sleep(1)
                #分行城市
                common.display_input('xpath,//div[@class="ivu-drawer-wrap"]//div[@class="ivu-row"]'
                '/div[1]//input[@class="ivu-input ivu-input-default"]','testcity',4)
                time.sleep(1)
                #银行卡号
                common.display_input('xpath,//div[@class="ivu-drawer-wrap"]//div[@class="ivu-row"]'
                '/div[1]//input[@class="ivu-input ivu-input-default"]',pub_method.get_purerange(14,'number'),5)
                time.sleep(1)
                #上传银行图片
    ...
    # #出金
    # def withdrawal_action(self,traccount):
    #     try:
    #         #出金
