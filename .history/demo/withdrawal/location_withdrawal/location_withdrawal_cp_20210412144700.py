...
class location_withdrawal_incp():
    ...
    def usable_withdrawal(self):
        try:
            ...
            time.sleep(2)
            withdrawal_list=['localCurrency-card','bank-card','e-wallet-card']
            for i in withdrawal_list:
                if common.ele_is_displayed('css,.{} .isApprove'.format(i),2):
                    if i=='localCurrency-card':
                        self.withdrawal_text=common.display_get_text('css,.localCurrency .paymentInfo-title')
                        return self.withdrawal_text
                        break
                    elif i=='bank-card':
                        self.withdrawal_text=common.display_get_text('css,.internationalBankWire .paymentInfo-title')
                        return self.withdrawal_text
                        break
                    else:
                        self.withdrawal_text=common.display_get_text('css,.e-wallet .paymentInfo-title')
                        break
                        return self.withdrawal_text
                else:
                    print('该账号不存在出金启用状态的出金渠道')
        except Exception as msg:
