    ...
    def withdrawal_action(self,traccount,amount):
        ...
        try:
            ...
            if amount>=self.withdrawal_amount:
                print('取款金额大于可取款金额，更改出金金额为可取款金额的1/2')
                print(type(self.withdrawal_amount))
                common.display_input('xpath,//div[@class="left-row-content el-row"]'
                '//input[@class="el-input__inner"]',self.withdrawal_amount/2)
            else:
                common.display_input('xpath,//div[@class="left-row-content el-row"]'
                '//input[@class="el-input__inner"]',amount)
            time.sleep(1)
            #输入验证码
