...
class locations_of_deposit():
    ...
    #判断交易账户是否满足入金条件
    def is_tdaccount_can_deposit(self,account):
        try:
            #获取该交易账号在交易列表中的行数
            self.rows=self.status_and_deposit_is_selected(account)
            print(self.rows)
            #判断交易账号状态
            if common.get_text('xpath,//*[@id="tdAccount"]/div[2]/div/div/div[3]/'
            'div[1]/div[2]/table/tbody/tr[{}]/td[5]/div/div/span'.format(self.rows)) in ('停用','Closed'):
                time.sleep(3)
                #状态为停用时，更改交易账号状态
                print(common.ele_is_displayed('xpath,//*[@id="tdAccount"]/div[2]/div/div/div[3]/div[1]/div[2]/table/tbody/tr[2]/td[7]/div/div/div/div/a',2))
                common.web_click('css,div.ivu-table-fixed-body>table>tbody>tr>td>div>div>div>div>a',3)
                #common.web_click('xpath,//*[@id="tdAccount"]/div[2]/div/div/div[3]/div[1]/div[2]/table/tbody/tr[2]/td[7]/div/div/div/div/a',1)
            else:
                pass

                
                
div.ivu-table-fixed-body>table>tbody>tr>td>div>div>div>div>a
