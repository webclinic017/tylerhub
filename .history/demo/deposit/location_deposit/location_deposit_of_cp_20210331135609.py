...
class locations_of_deposit():
    ...
    #判断交易账户状态
    def tdaccount_status(self,account):
        try:
            #获取该交易账号在交易列表中的行数
            self.rows=self.where_is_account(account)
            #判断交易账号状态
            ...
            if self.account_status in ('停用','Closed'):
                ...
                print('该交易账号状态为:{}'.format(self.account_status))
        except Exception as msg:
            pub_method.log_output('!!--!!is_tdaccount_can_deposit').error(msg)

    #判断交易账号入金按钮是否被勾选
    def deposti_is_selected(self):
        try:
            #获取该交易账号在交易列表中的行数
            #self.rows=self.where_is_account(account)
            #判断入金按钮是否被勾选
            print(common.is_element_selected('xpath,//*[@id="tdAccount"]/div[2]/div/div/div[3]/div[1]/div[2]/table/tbody/tr[3]/td[6]>div>div>div>label>span>span>inout'))
        except Exception as msg:
            pub_method.log_output('!!--!!deposti_is_selected').error(msg)


//*[@id="tdAccount"]/div[2]/div/div/div[3]/div[1]/div[2]/table/tbody/tr[3]/td[6]>div>div>div>label>span>span>inout
