    ...
    def is_tdaccount_can_deposit(self,account):
        try:
            ...
            if self.account_status in ('停用','Closed'):
                ...
                time.sleep(1)
                #选择激活或者暂停状态
                self.changetype_index=random.randint(0,1)
                common.display_click('css,.ivu-select-visible .ivu-select-dropdown-list > li',self.changetype_index) #0:激活 1：暂停
                time.sleep(1)
                if self.changetype_index==0:
                    print('更改交易账号状态为"激活"')
                else:
                    print('更改交易账号状态为"暂停"')
                #提交表单
