...
class locations_of_deposit():
    ...
    #BOS审核入金请求
    def review_deposit(self,bos_username,bos_psword,traccount,path,column,row):
        try:
            common.switch_windows(1)
            #资金管理
            ...
            time.sleep(5)
            #获取交易账户余额
            e.saveainfo(path,self.get_traccount_balance(traccount),column,row)
        except Exception as msg:
    ...
    def deposit_cp(self,traccount,username,psword,amount,bos_username,bos_psword,path,column,row):
        """
        满足入金条件后，会员中心入金并审核
        """
        try:
