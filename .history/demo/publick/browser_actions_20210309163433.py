...
class Commonweb():
    ...
    def display_find_elements(self,locator):
        ...
        try:
            if 'css' in method:
                return WebDriverWait(self.driver,10,0.5).until(lambda x:x.find_elements_by_css_selector(values))
            elif 'xpath' in method:
                return WebDriverWait(self.driver,10,0.5).until(lambda x:x.find_elements_by_xpath(values))
            elif 'name' in method:
                return WebDriverWait(self.driver,10,0.5).until(lambda x:x.find_elements_by_name(values))
            elif 'id' in method:
                return WebDriverWait(self.driver,10,0.5).until(lambda x:x.find_elements_by_id(values))
            elif 'class' in method:
                return WebDriverWait(self.driver,10,0.5).until(lambda x:x.find_elements_by_class_name(values))
            elif 'partial' and 'link' in method:
                return WebDriverWait(self.driver,10,0.5).until(lambda x:x.find_elements_by_partial_link_text(values))
            elif 'link' in method:
                return WebDriverWait(self.driver,10,0.5).until(lambda x:x.find_elements_by_link_text(values))
            elif 'tag' in method:
                return WebDriverWait(self.driver,10,0.5).until(lambda x:x.find_elements_by_tag_name(values))
            else:
                pub_method.log_output('!!--!!location_element').info('定位方法错误')
        except Exception as msg:
