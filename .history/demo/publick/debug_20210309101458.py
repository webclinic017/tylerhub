...
# c.open_web('https://www.baidu.com')
# time.sleep(1)
# c.web_click('css,.title-content-title',-1)
# c.switch_windows(0)
# # c.close_browser()
# # c.switch_windows(0)
from selenium import webdriver
...
def display(elment):
    return WebDriverWait(dr,10,1).until(lambda x:x.find_elements_by_css_selector(elment))


# change=WebDriverWait(dr,10,1).until(lambda x:x.find_element_by_css_selector())
# change=WebDriverWait(dr,10,1).until(lambda x:x.find_element_by_css_selector())
#display('.title-content-title')[0].click()
print(display('.title-content-title'))