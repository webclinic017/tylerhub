...
class exceldata():
    ...
    # 创建存储注册数据的函数，写入已存在的本地文档中,clo:列；row:行
    def saveaccount(self,excelpath,val,col,row):
        """注：调用此函数时，不能打开需要写入数据的文档"""
        self.workbook = load_workbook(filename=self.excelpath)
        self.sheet = workbook.active
        self.cel = sheet['{}{}'.format(self.col, self.row)]  
        self.cel.value = self.val
        self.workbook.save(filename=excelpath)
...
if __name__=='__main__':
    ...
    print(a)
    print(a[0]['邮箱'][0:2]=='14')
    e.saveaccount(r'E:\test\account_number.xlsx','测试','F','5')
