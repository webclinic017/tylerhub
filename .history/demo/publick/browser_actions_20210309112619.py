...
class Commonweb():
    ...
    #显示等待查找页面元素,timeout=10s，每1s询问一次
    def display_find_elements(self,locator):
        #切片
        method,values=locator.split(',',maxsplit=1)
        try:
            if 'css' in method:
                return WebDriverWait(self.driver,10,1).until(lambda x:x.find_elements_by_css_selector(values))
            elif 'xpath' in method:
                return WebDriverWait(self.driver,10,1).until(lambda x:x.find_elements_by_xpath(values))
            elif 'name' in method:
                return WebDriverWait(self.driver,10,1).until(lambda x:x.find_elements_by_name(values))
            elif 'id' in method:
                return WebDriverWait(self.driver,10,1).until(lambda x:x.find_elements_by_id(values))
            elif 'class' in method:
                return WebDriverWait(self.driver,10,1).until(lambda x:x.find_elements_by_class_name(values))
            elif 'partial' and 'link' in method:
                return WebDriverWait(self.driver,10,1).until(lambda x:x.find_elements_by_partial_link_text(values))
            elif 'link' in method:
                return WebDriverWait(self.driver,10,1).until(lambda x:x.find_elements_by_link_text(values))
            elif 'tag' in method:
                return WebDriverWait(self.driver,10,1).until(lambda x:x.find_elements_by_tag_name(values))
            else:
                pub_method.log_output('!!--!!location_element').info('定位方法错误')
        except Exception as msg:
    ...
    def display_find_element(self,locator,index=0):
        self.display_elements=self.display_find_elements(locator,index)
        try:
    ...
    #显示等待双击
    def displayed_double_click(self,element):
        try:
            self.double=self.display_findelement(element)
            ActionChains(self.driver).double_click(self.double).perform()
        except Exception as msg:
            pub_method.log_output('!!--!!displayed_double_click').error(msg)
