...
class location_of_transfer():
    ...
    def transfer_action(self,tdaccount_from,tdaccount_to,amount):
        try:
            ...
            common.display_click('css,.el-button--primary > span',-1)#bug弹窗
            ...
            if self.transfer_balance >= int(amount):#可转金额大于转账金额
                common.display_input('xpath,//input[@class="el-input__inner"]',int(amount),2)
                time.sleep(1)
    ...
    def transfer_in_cp(self,username,psword,tdaccount_from,tdaccount_to):
        try:
            self.logincp(username,psword)
            time.sleep(8)
            self.transfer_action(tdaccount_from,tdaccount_to)
        except Exception as msg:
