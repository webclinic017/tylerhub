...
class location_of_transfer():
    ...
    def transfer_action(self,tdaccount_from,tdaccount_to,amount):
        try:
            #获取转出交易账号余额
            ...
            time.sleep(1)
            #下一步
            common.display_click('css,.next > span')
        except Exception as msg:
    ...
    def is_auto_review(self):
        try:
            ...
            time.sleep(1)
            #判断自动转账是否开启
            common.display_click('xpath,//*[@id="app"]/div/div/div[4]/div/div[1]/div/div[2]/div/button[5]/span')#设置
            time.sleep(1)
            #开启按钮是否被勾选
            if common.is_element_selected('css,.ivu-card-extra .ivu-radio-checked > .ivu-radio-input'):
                print('自动转账已开启')
                return True
            else:
                return False
        except Exception as msg:
            pub_method.log_output('!!--!!is_auto_review').error(msg)





    def transfer_in_cp(self,username,psword,tdaccount_from,tdaccount_to,amount):
        try:
            self.logincp(username,psword)
            time.sleep(10)
            self.transfer_action(tdaccount_from,tdaccount_to,amount)
