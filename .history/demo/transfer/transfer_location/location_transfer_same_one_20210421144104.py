...
class location_of_transfer():
    ...
    def where_is_tdaccount_cp(self,tdaccount):
        ...
        for y in range(0,self.len_incp):
            if pub_method.extract_numbers(common.get_text('css,.account-number-cla',y)) == new_str:
                return i+1
                break
    ...
    def is_status_five(self):
        self.status_num=0
        for x in range(1,self.tdaccount_list_len+1):
            self.status=common.display_get_text('xpath,//*[@id="tdAccount"]/div[2]/div/div/div[3]/'
            'div[1]/div[2]/table/tbody/tr[{}]/td[5]/div/div/span'.format(x))
            time.sleep(1)
    ...
    def transfer_to_lower(self):
        try:
            ...
            time.sleep(1)
            for a in range(5,7):
                if common.get_attributes('css,label.switch>span.ivu-switch>input','value',a)=='true':#转账给下级客户控制
                    time.sleep(1)
                    common.display_click('css,label.switch>span.ivu-switch',a)
                    time.sleep(1)
    ...
    def tdaccount_status(self):
        try:
            for z in [self.from_site,self.to_site]:
                self.tdaccount_status=common.display_get_text('xpath,//*[@id="tdAccount"]/div[2]/div/div/div[3]/'
                'div[1]/div[2]/table/tbody/tr[{}]/td[5]/div/div/span'.format(z))
                time.sleep(2)
                if self.tdaccount_status in ('停用','Closed'):
                    #状态为停用时，更改交易账号状态
                    common.display_click('css,div.ivu-table-fixed-body>table>tbody>tr>td>div>div>div>div>a',2*z-1) #编辑
                    time.sleep(1)
    ...
    def transfer_is_selected(self):
        try:
            ...
            self.is_status_five()
            time.sleep(1)
            for p in [self.from_site,self.to_site]:
                if not common.is_element_selected('xpath,//*[@id="tdAccount"]/div[2]/div/div/div[3]/div[1]'
                '/div[2]/table/tbody/tr[{}]/td[6]/div/div/div/label/span/input'.format(p),-1):#未被勾选时
                    common.display_click('css,div.ivu-table-fixed-body>table>tbody>tr>td>div>div>div>div>a',2*p-1) #编辑
                    time.sleep(1)
                    #勾选转账权限
    ...
    def is_satisfy_transfer(self,account,tdaccount_from,tdaccount_to):
        try:
            ...
            print('转出：{}'.format(self.from_site))
            self.to_site=self.where_is_tdaccount_bos(tdaccount_to)
            print('转入：{}'.format(self.to_site))
            time.sleep(1)
            #交易账号状态
    ...
    def transfer_in_cp(self,username,psword,account,tdaccount_from,tdaccount_to,amount,excelpath,row):
        try:
            ...
            #保存
            print('转账成功后转出交易账号余额为：{}'.format(self.after_transfer_balance))
