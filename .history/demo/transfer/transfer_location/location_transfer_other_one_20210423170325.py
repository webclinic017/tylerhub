...
class locathion_of_transfer():
    ...
    def is_status_five(self):
        try:
            time.sleep(1)
            self.activation=int(pub_method.extract_numbers(common.display_get_text('css,.statusCount > span')))
            time.sleep(1)
            self.puase=int(pub_method.extract_numbers(common.display_get_text('css,.statusCount > span',1)))
