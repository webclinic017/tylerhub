...
class Test_transfer_other_account():
    ...
    def test_transfer(self,data):
        ...
        int(data['转出交易账号']),int(data['转入主账号']),float(data['转账金额'])):
            pytest.skip()
            print('转出交易账户余额为0，跳过此用例')
        else:
            print(transfer.transfer_incp(data['邮箱'], data['密码'],int(data['转入主账号']),int(data['转出交易账号']),int(data['转入主账号']),float(data['转账金额'])))
            print('继续执行用例')
