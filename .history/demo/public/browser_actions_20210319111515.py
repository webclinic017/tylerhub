...
class Commonweb():
    ...
    def coed_img(self,name,filename,locator,index=0):
        try:
            ...
            #打开之前的截屏
            print(os.path.join(self.pictdir_path,'{}{}'.format(name,self.pict_name)))
            # self.image=Image.open(os.path.join(self.pictdir_path,'{}{}'.format(name,self.pict_name)))
            # #裁剪截图，裁剪范围为之前获取的左上角至右下角区域
            # self.new_image=self.image.crop((self.left,self.lefty,self.right,self.righty))
            # #保存裁剪的图片
            # self.new_image.save(os.path.join(self.pictdir_path,'{}code{}'.format(name,self.pict_name)))
        except Exception as msg:
