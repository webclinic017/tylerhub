...
class commonmethod():
    ...
    def choose_bos_lang(self,lang):
        try:
            if lang=='CN' or lang=='简中':
                ...
                time.sleep(1)
                self.driver.find_element_by_xpaht('//li[@class="ivu-select-item ivu-select-item-selected"]').click()
            else:
                pass
        except Exception as msg:
    ...
    def loginbos(self,username,psword):
        try:
            ...
            time.sleep(1)
            self.driver.driver.find_element_by_css_selector('.ivu-btn-large').click()
        except Exception as msg:
