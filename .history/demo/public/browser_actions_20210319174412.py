...
class Commonweb():
    ...
    def open_browser(self,browsername='Chrome'):
        ...
        try:
            if browsername=='firefox' or browsername=='Firefox' or browsername=='fx': #火狐浏览器
                ...
                self.driver.implicitly_wait(10) #隐式等待20s
            else:
    ...
    #设置等待时间查找页面元素是否可见
    def is_displayed(self,locator,timeout):
        """
        ...
        try:
            if 'css' in method:
                try:
                    WebDriverWait(self.driver,timeout,1).until(EC.presence_of_element_located((By.CSS_SELECTOR,values)))
                    return True
                except:
                    return False
            elif 'xpath' in method:
                try:
                    WebDriverWait(self.driver,timeout,1).until(EC.presence_of_element_located((By.XPATH,values)))
                except:
                    return False
            elif 'id' in method:
                try:
                    WebDriverWait(self.driver,timeout,1).until(EC.presence_of_element_located((By.ID,values)))
                except:
                    return False
            elif 'name' in method:
                try:
                    WebDriverWait(self.driver,timeout,1).until(EC.presence_of_element_located((By.NAME,values)))
                except:
                    return False
            else:
