...
class Commonweb():
    ...
    #截取屏幕，抓取验证码图片
    def coed_img(self,name,filename,locator,index=0):
        try:
            # """
            # name:截图名称
            ...
            self.driver.get_screenshot_as_file(path+r'\{}\{}.png'.format(filename,name))
            #获取图片元素定位
            ...
            #打开之前的截图
            image=Image.open(path+r'\{}\{}.png'.format(filename,name))
            #对图片进行裁剪，裁剪范围为之前验证的左上角至右下角范围
            new_image=image.crop((left,lefty,right,righty))
            new_image.save(path+r'\{}\code.png'.format(filename))
        except Exception as msg:
