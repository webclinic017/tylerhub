...
class Commonweb():
    ...
    #截取屏幕，抓取验证码图片
    def coed_img(self,name,locator,index=0,filename='codeimg'):
        try:
            # """
            # name:截图名称
            # filename:保存截图的文件夹，默认在当前路径下
            # """
            # #截取当前屏幕
            # self.get_screenpict(name,filename)
            # #获取验证码图片元素定位
            # self.img_ele=self.find_element(locator,index)
            # #得到验证码图片左上角及右下角的xy左坐标
            # self.left=self.img_ele.location.get('x')
            # self.lefty=self.img_ele.location.get('y')
            # self.right=self.left+self.img_ele.size.get('width')
            # self.righty=self.right+self.img_ele.size.get('height')
            # #打开之前的截屏
            # self.image=Image.open(self.picture)
            # #裁剪截图，裁剪范围为之前获取的左上角至右下角区域
            # self.new_image=self.image.crop((self.left,self.lefty,self.right,self.righty))
            # #保存裁剪的图片
            # self.new_image.save(r'E:\tyler96qa\tylerhub\demo\registration_process\register_positioning\codeimg\code2.png')
            


            self.driver.get_screenshot_as_file(r'E:\tyler96qa\tylerhub\demo\registration_process\register_positioning\codeimg\code.png')
            #获取图片元素定位
            img_ele=self.find_element('css,.s_btn')
            ...
            #打开之前的截图
            image=Image.open(r'E:\tyler96qa\tylerhub\demo\registration_process\register_positioning\codeimg\code.png')
            #对图片进行裁剪，裁剪范围为之前验证的左上角至右下角范围
            new_image=image.crop((left,lefty,right,righty))
            new_image.save(r'E:\tyler96qa\tylerhub\demo\registration_process\register_positioning\codeimg\code2.png')
        except Exception as msg:
