...
class Commonweb():
    ...
    def is_displayed(self,locator,timeout):
        ...
        try:
            if 'css' in method:
                try:
                    WebDriverWait(self.driver,timeout,1).until(EC.visibility_of_any_elements_located((By.CSS_SELECTOR,values)))
                    return True
                except:
                    return False
            elif 'xpath' in method:
                try:
                    WebDriverWait(self.driver,timeout,1).until(EC.visibility_of_elements_located((By.XPATH,values)))
                except:
                    return False
            elif 'id' in method:
                try:
                    WebDriverWait(self.driver,timeout,1).until(EC.visibility_of_elements_located((By.ID,values)))
                except:
                    return False
            elif 'name' in method:
                try:
                    WebDriverWait(self.driver,timeout,1).until(EC.visibility_of_elements_located((By.NAME,values)))
                except:
                    return False
            else:
