...
class Location_of_transfer():
    ...
    def tdaccount_to_is_rebate(self,tdaccount_to):
        try:
            ...
            log.my_logger('!!--!!tdaccount_to_is_rebate').error(msg)
    
    #判断转入转出账号状态是否满足转账条件
    def tdaccount_status_is_satisfy(self):
        try:
            for i in [self.from_site,self.to_site]:
                self.is_status_five() #获取当前交易账号激活+暂停状态总数
    ...
    def is_satisfy_transfer(self,account,tdaccount_from,tdaccount_to):
        try:
            ...
            print('转出交易账号位于交易列表中位置：{}'.format(self.from_site))
            self.to_site=self.where_is_tdaccount_bos(tdaccount_to)
            print('转入交易账号位于交易列表中位置：{}'.format(self.to_site))
            time.sleep(1)
            #交易账号状态
            self.tdaccount_status_is_satisfy()
            time.sleep(1)
            #判断交易账号转账按钮是否被勾选
            ...
            log.my_logger('!!--!!is_satisfy_transfer').error(msg)
    ...
    def transfer_action(self,tdaccount_from,tdaccount_to,amount,excelpath,row):
        try:
            ...
            log.my_logger('!!--!!transfer_action').error(msg)
    ...
    def is_successful_transfer(self):
        try:
            ...
            log.my_logger('!!--!!is_successful_transfer').error(msg)
    ...
    def transfer_in_cp(self,username,psword,account,tdaccount_from,tdaccount_to,amount,excelpath,row):
        try:
            ...
            log.my_logger('!!--!!transfer_in_cp').error(msg)
