...
LastEditTime: 2022-05-31 11:34:42
...
class Location_of_IB_Open_OrderList(object):
    ...
    def filter_lower_openOrder(self):
        try:
            ...
            print('当前账号存在{}条下级未平仓订单记录'.format(self.lenForOpenOrder))
            #遍历IB下级列表，找出存在未平仓记录的账号
            self.lower_list=[]
            print('筛选下级未平仓账号中，loading......')
    ...
    def search_period_openOrder(self,tradeAccount):
        try:
            ...
            print('数据库查询返回{}条未平仓数据'.format(len(self.mysql_openOrder)))
    ...
    def search_buyOrSale_openOrder(self,tradeAccount):
        try:
            ...
            if self.type_index==1:
                self.tradeType='买'
                self.mysql_openOrder=dataBase.search_in_mysql('SELECT * FROM report_atfx2_test.mt4_sync_order WHERE Login="{}" and cmd=0 and Close_Time="1970-01-01 00:00:00" and'
                ' Open_Time between "{} 00:00:00" and "{} 23:59:59" order by Ticket desc'.
                format(tradeAccount,self.dateStart,self.dateEnd), conFig.get_value('mysql_AWS', 'host'), conFig.get_value('mysql_AWS','user'),conFig.get_value('mysql_AWS','password'),type='all') 
                print('数据库查询返回{}条类型为{}的数据'.format(len(self.mysql_openOrder),self.tradeType))
            else:
                self.tradeType='卖'
                 #卖
                self.mysql_openOrder=dataBase.search_in_mysql('SELECT * FROM report_atfx2_test.mt4_sync_order WHERE Login="{}" and cmd=1 and Close_Time="1970-01-01 00:00:00" and'
                ' Open_Time between "{} 00:00:00" and "{} 23:59:59" order by Ticket desc'.
                format(tradeAccount,self.dateStart,self.dateEnd), conFig.get_value('mysql_AWS', 'host'), conFig.get_value('mysql_AWS','user'),conFig.get_value('mysql_AWS','password'),type='all')
                print('数据库查询返回{}条类型为{}的数据'.format(len(self.mysql_openOrder),self.tradeType))
    ...
    def search_ticket_openOrder(self,ticket):
        try:
            ...
            #查询数据库
            self.mysql_openOrder=dataBase.search_in_mysql('SELECT * FROM report_atfx2_test.mt4_sync_order WHERE Ticket="{}" and Close_Time="1970-01-01 00:00:00" and'
            ...
            #处理页面数据
            self.open_orderDict={}
            self.open_orderDict['Ticket']=int(common.get_text('css,tbody > tr > .el-table_1_column_1 div span')) #订单号码
            self.open_orderDict['Login']=int(common.get_text('css,tbody > tr > .el-table_1_column_3 .account-txt')) #交易账号
            ...
            self.open_orderDict['Volume']=float(common.get_text('css,tbody > tr > .el-table_1_column_10 div span')) #交易量
            self.open_orderDict['Symbol']=common.get_text('css,tbody > tr > .el-table_1_column_11 div span') #商品代码
            self.open_orderDict['Open_Price']=float(common.get_text('css,tbody > tr > .el-table_1_column_12 div span')) #开仓价
            self.open_orderDict['Commission']=locale.atof(common.get_text('css,tbody > tr > .el-table_1_column_15 div span')) #佣金
            self.open_orderDict['Swaps']=locale.atof(common.get_text('css,tbody > tr > .el-table_1_column_16 div span')) #隔夜利息
            self.open_orderDict['Profit']=locale.atof(common.get_text('css,tbody > tr > .el-table_1_column_17 div span')) #盈亏
            return self.open_orderDict
        except Exception as msg:
            log.my_logger('!!--!!search_ticket_openOrder').error(msg)
        

    #查询交易账号复制数据是否与数据库一致
    def copy_open_export_OrderList(self,tradeAccount):
        try:
            #筛选时间-全部
            common.display_click('css,.el-select .el-select__caret',1)
            time.sleep(0.5)
            ...
            for root,dirname,filename in os.walk(os.path.join(path_project,'test_data')):
                for i in filename:
                    if re.findall('Sub_Open_Trade_List_...........xlsx',i)!=[]:
                        self.openOrder_excel=''.join(re.findall('Sub_Open_Trade_List_...........xlsx',i)) #正则匹配下载文件名
                        break
                    else:
                        pass
            ...
            log.my_logger('!!--!!copy_open_export_OrderList').error(msg)
