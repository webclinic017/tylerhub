...
LastEditTime: 2022-05-24 11:27:34
...
class Test_deposti_withdrawal_list(object):
    ...
    global verifyList,conFig,dealData,testdata,excelpath,ranDom
        ...
        for i in range(0,2):
            ...
            with allure.step('判断当前时间段是否存在出入金转账记录'):
                if verifyList.serch_list(int(data['主账号'])):
                    with allure.step('测试账号：{}在该时间段存在出入金转账记录'.format(int(data['主账号']))):
                        ...
                        with allure.step('判断当前时间段是否存在出金记录'):
                            if verifyList.search_withdrawal(int(data['主账号'])):
                                ...
                                with allure.step('存在出金记录{}条，断言页面每条出金记录是否与数据库一致'.format(verifyList.withdrawal_len)):
                                    ...
                                    with allure.step('断言出金条数，数据库{}条，页面{}条'.format(len(verifyList.mongodbWithdrawal),verifyList.withdrawal_len)):   
                                    #断言页面出金条数是否与数据库一致
                                        check.equal(len(verifyList.mongodbWithdrawal),verifyList.withdrawal_len,
                                        '断言失败：数据库出金记录{}条，页面出金记录{}条'.format(len(verifyList.mongodbWithdrawal),verifyList.withdrawal_len))
                                        
                                    
                                    for i in range(0,ranDom.return_small(len(verifyList.mongodbWithdrawal),verifyList.withdrawal_len)):
                                    #断言每条出金记录时间、交易金额、汇率、mt金额、状态、出金渠道是否与数据库一致
                                        ...
                                        verifyList.withdrawal_list[i]['createDate_mt'].strftime('%Y-%m-%d %H:%M'))):
                                            ...
                                            verifyList.withdrawal_list[i]['createDate_mt'].strftime('%Y-%m-%d %H:%M'),
                                            '第{}条数据断言失败：数据库出金时间 {}，页面出金时间 {}'.format(i+1,verifyList.mongodbWithdrawal[i]['createDate_mt'].strftime('%Y-%m-%d %H:%M'),
                                            verifyList.withdrawal_list[i]['createDate_mt'].strftime('%Y-%m-%d %H:%M')))
                                            
                                        with allure.step('断言交易金额：数据库出金交易金额 {}；页面出金交易金额 {}'.format(verifyList.mongodbWithdrawal[i]['settleAmt'],
                                        verifyList.withdrawal_list[i]['settleAmt'])):
                                            ...
                                            '第{}条数据断言失败：数据库出金交易金额：{};页面出金交易金额：{}'.format(i+1,verifyList.mongodbWithdrawal[i]['settleAmt'],verifyList.withdrawal_list[i]['settleAmt']))
                                        ...
                                        verifyList.withdrawal_list[i]['realRate'])):
                                            ...
                                            '第{}条数据断言失败：数据库出金汇率：{};页面出金汇率：{}'.format(i+1,verifyList.mongodbWithdrawal[i]['realRate'],verifyList.withdrawal_list[i]['realRate']))
                                        ...
                                        verifyList.withdrawal_list[i]['mtAmt'])):
                                            ...
                                            '第{}条数据断言失败：数据库出金mt金额：{};页面出金mt金额：{}'.format(i+1,verifyList.mongodbWithdrawal[i]['mtAmt'],verifyList.withdrawal_list[i]['mtAmt']))
                                        ...
                                        verifyList.withdrawal_list[i]['currStatus'])):
                                            check.is_in(verifyList.mongodbWithdrawal[i]['currStatus'],'SUL')
                                            check.is_in(verifyList.withdrawal_list[i]['currStatus'],'成功待审核')
                                        
                                        with allure.step('断言出金渠道：数据库出金渠道 {}'.format(verifyList.mongodbWithdrawal[i]['channel'])):
                                            check.is_in(verifyList.mongodbWithdrawal[i]['channel'],self.withdrawal_type,'出金渠道断言失败'.format(verifyList.mongodbWithdrawal[i]['channel']))
                            else:
                                with allure.step('不存在出金记录，断言当前时间段内数据库查询数据是否为空'):
                                    check.equal(verifyList.mongodbWithdrawal,[],'断言失败，数据库出金查询不为空:{}'.format(verifyList.mongodbWithdrawal))
                                    print('当前时间段出金记录为空')
                        ...
                        with allure.step('判断当前时间段是否存在入金记录'):
                            if verifyList.search_deposit(int(data['主账号'])):
                                ...
                                with allure.step('存在入金记录{}条，断言页面每条入金记录是否与数据库一致'.format(verifyList.deposit_len)):
                                    ...
                                    'jpayAli', 'neteller2', 'walaopayMYR', 'payMeroUSDTTRC20','bankwire','']

                                    #断言页面入金金条数是否与数据库一致
                                    with allure.step('断言入金条数，数据库{}条，页面{}条'.format(len(verifyList.mongodbDeposit),verifyList.deposit_len)):
                                        check.equal(len(verifyList.mongodbDeposit),verifyList.deposit_len,
                                        '断言失败，数据库入金记录{}条，页面入金记录{}条'.format(len(verifyList.mongodbDeposit),verifyList.deposit_len),)

                                    for i in range(0,ranDom.return_small(len(verifyList.mongodbDeposit),verifyList.deposit_len)):
                                        ...
                                        verifyList.deposti_list[i]['createDate_mt'].strftime('%Y-%m-%d %H:%M'))):
                                            ...
                                            verifyList.deposti_list[i]['createDate_mt'].strftime('%Y-%m-%d %H:%M'),
                                            '第{}条数据断言失败：数据库入金时间 {}，页面入金时间 {}'.format(i+1,verifyList.mongodbDeposit[i]['createDate_mt'].strftime('%Y-%m-%d %H:%M'),
                                            verifyList.deposti_list[i]['createDate_mt'].strftime('%Y-%m-%d %H:%M')))
                                        ...
                                        verifyList.deposti_list[i]['fromAmt'])):
                                            ...
                                            '第{}条数据断言失败：数据库入金交易金额：{};页面入金交易金额：{}'.format(i+1,verifyList.mongodbDeposit[i]['fromAmt'],verifyList.deposti_list[i]['fromAmt']))
                                        ...
                                        verifyList.deposti_list[i]['rate'])):
                                            ...
                                            '第{}条数据断言失败：数据库入金交易汇率：{};页面入金交易汇率：{}'.format(i+1,verifyList.mongodbDeposit[i]['rate'],verifyList.deposti_list[i]['rate']))
                                        ...
                                        verifyList.deposti_list[i]['mt4Amt'])):
                                            ...
                                            '第{}条数据断言失败：数据库交易入金mt金额：{};页面交易入金mt金额：{}'.format(i+1,verifyList.mongodbDeposit[i]['mt4Amt'],verifyList.deposti_list[i]['mt4Amt']))

                                        with allure.step('断言入金状态：数据库入金状态 {}；页面入金状态 {}'.format(verifyList.mongodbDeposit[i]['currStatus'],
                                        verifyList.deposti_list[i]['currStatus'])):
                                            check.is_in(verifyList.mongodbDeposit[i]['currStatus'],'SUL')
                                            check.is_in(verifyList.deposti_list[i]['currStatus'],'成功待审核')

                                        with allure.step('断言入金渠道：数据库入金渠道 {}'.format(verifyList.mongodbDeposit[i]['channel'])):
                                            check.is_in(verifyList.mongodbDeposit[i]['channel'],self.deposit_type,'入金渠道断言失败：{}'.format(verifyList.mongodbDeposit[i]['channel']))
                            else:
                                with allure.step('不存在入金记录，断言当前时间段内数据库查询数据是否为空'):
                                    check.equal(verifyList.mongodbDeposit,[],'断言失败，数据库入金记录查询不为空：{}'.format(verifyList.mongodbDeposit))
                                    print('当前时间段入金记录为空')
                        ...
                        with allure.step('判断当前时间段是否存在转账记录'): 
                            if verifyList.search_transfer(int(data['主账号'])):
                                with allure.step('存在转账记录，断言页面每条转账记录是否与数据库一致'):
                                    ...
                                    with allure.step('断言转账条数，数据库{}条，页面{}条'.format(len(verifyList.mongodbTransfer),verifyList.transfer_len)):
                                        check.equal(len(verifyList.mongodbTransfer),verifyList.transfer_len,
                                        '断言失败，数据库转账记录{}条，页面转账记录{}条'.format(len(verifyList.mongodbTransfer),verifyList.transfer_len))
                                        
                                        for i in range(0,ranDom.return_small(len(verifyList.mongodbTransfer),verifyList.transfer_len)):
                                        #断言每条转账记录时间、mt金额、状态是否与数据库一致
                                            ...
                                            verifyList.transfer_list[i]['createDate_mt'].strftime('%Y-%m-%d %H:%M'))):
                                                ...
                                                verifyList.transfer_list[i]['createDate_mt'].strftime('%Y-%m-%d %H:%M'),
                                                '第{}条数据断言失败：数据库转账时间 {}，页面转账时间 {}'.format(i+1,verifyList.mongodbTransfer[i]['createDate_mt'].strftime('%Y-%m-%d %H:%M'),
                                                verifyList.transfer_list[i]['createDate_mt'].strftime('%Y-%m-%d %H:%M')))
                                        ...
                                        verifyList.transfer_list[i]['toMtAmt'])):
                                            ...
                                            '第{}条数据断言失败：数据库转账交易mt金额：{};页面转账交易mt金额：{}'.format(i+1,verifyList.mongodbTransfer[i]['toMtAmt'],verifyList.transfer_list[i]['toMtAmt']))
                                ...
                                with allure.step('不存在转账记录，断言数据库数据库是否为空'):
                                    check.equal(verifyList.mongodbTransfer,[],'断言失败，数据库转账记录查询不为空：{}'.format(verifyList.mongodbTransfer))
                                    print('当前时间段转账记录为空')
                else:
                    with allure.step('当前时间段不存在出入金转账记录,断言数据库查询是否为空'):
                        #断言数据库查询是否为空
                        check.equal(verifyList.mongodbWithdrawal,[],'断言失败：数据库入金记录查询不为空:{}'.format(verifyList.mongodbWithdrawal))
                        check.equal(verifyList.mongodbDeposit,[],'断言失败：数据库出金记录查询不为空:{}'.format(verifyList.mongodbDeposit))
                        check.equal(verifyList.mongodbTransfer,[],'断言失败：数据库转账记录查询不为空:{}'.format(verifyList.mongodbTransfer))
                        print('当前时间段出入金记录为空')
